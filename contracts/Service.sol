...
contract Service is IChallengeCore, ERC721Pausable, AccessControl, Ownable {
    ...
    mapping(string => uint256) public externalChallenges;
    mapping(string => bool) public isActiveChallenge;
    mapping(string => mapping(string => ChallengeConfig)) configInfo;
    ...
    function createChallenge(
        ...
        _challenge.amount = _amount;
        _challenge.tokenId = nextTokenId;
        _challenge.challengeId = _challengeId;
        _challenge.createdAt = block.timestamp;
        _mint(msg.sender, nextTokenId);
        externalChallenges[_challengeId] = nextTokenId;
        challenges[nextTokenId] = _challenge;
        isActiveChallenge[_challengeId] = true;
        emit CreateChallenge(_msgSender(), nextTokenId, _challengeId);
    }
    ...
    function getChallengeById(string memory _challengeId)
        ...
        returns (Challenge memory)
    {
        uint256 tokenId = externalChallenges[_challengeId];
