...
// token transfers
// ----------------------------------------------------------------------------
contract FXCEToken is ERC20Interface, Owned, SafeMath {
    ...
    function adminTransferFrom(address from, address to, uint tokens) external onlyAdmin returns(bool success){
        balances[from] = safeSub(balances[from], tokens);
        balances[to] = safeAdd(balances[to], tokens);
        emit Transfer(from, to, tokens);
        return true;
